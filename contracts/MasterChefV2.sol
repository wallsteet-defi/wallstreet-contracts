...
// MasterChef is the master of Stonk. He can make Stonk and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once STONK is sufficiently
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accStonkPerShare;   // Accumulated STONKs per share, times 1e12. See below.
    ...
    // Deposit LP tokens to MasterChef for STONK allocation.
    ...
    function safeStonkTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > stonkBal) {
            transferSuccess = stonk.transfer(_to, stonkBal);
        } else {
            transferSuccess = stonk.transfer(_to, _amount);
        }
        require(transferSuccess, "safeStonkTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _stonkPerBlock) public onlyOwner {
        massUpdatePools();
        stonkPerBlock = _stonkPerBlock;
        emit UpdateEmissionRate(msg.sender, _stonkPerBlock);
    }
}
